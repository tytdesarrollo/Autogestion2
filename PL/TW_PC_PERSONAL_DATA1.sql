...
-- PARAMETROS DE ENTRADA
   ...
   -- ROL               : EL TIPO DE ROL DEFINE QUE BLOQUE SE DEBE VISUALIZAR A CADA USUARIO, POR EL MOMENTO EL ROL 1 ES EL ROL JEFE QUE TIENE DERCHO A TODOS LOS BLOQUES, Y EL ROL 2 SERIA PARA 
                        UN USUARIO TIPO SENA:
                        1 - TODOS LOS BLOQUES  
                        2 - TIPO SENA BLOQUE 1, 2                             
...
(
    ...
    ROL IN VARCHAR2, 
    BLOQUE1 OUT VARCHAR2, BLOQUE2 OUT VARCHAR2, BLOQUE3 OUT VARCHAR2, BLOQUE4 OUT VARCHAR2, 
    BLOQUE5 OUT VARCHAR2, BLOQUE6 OUT VARCHAR2, BLOQUE7 OUT VARCHAR2, BLOQUE8 OUT VARCHAR2,
    BLOQUE9 OUT VARCHAR2, BLOQUE10 OUT VARCHAR2, BLOQUE11 OUT VARCHAR2, BLOQUE12 OUT VARCHAR2,
    BLOQUE13 OUT VARCHAR2, BLOQUE14 OUT VARCHAR2
)
IS
...
PROCE_RETEFUEN                   VARCHAR(5); -- PROCEDIMIENTO RETENCION  EN LA FUENTE
...
CAJA_COMP                             VARCHAR(25);--------- VARIABLES AFILIACIONES - SEGURIDAD SOCIAL
...
BEGIN         
    ...
    BEGIN               
        BEGIN
            SELECT (B.NOM_EPL||' '||B.APE_EPL), CAR.NOM_CAR, CONT.NOM_CTO, B.FEC_ING, CASE WHEN GR.COD_GRU='2' THEN 'INTEGRAL' ELSE 'BÁSICO' END  
            INTO NOMBRE_EPL, CARGO, CONTRATO,FEC_INGRESO, TIP_SALARIO
            FROM EMPLEADOS_BASIC B, CARGOS CAR, CONTRATOS CONT, EPL_GRUPOS GR
            WHERE B.COD_EPL=CODIGO_EPL AND B.COD_CAR=CAR.COD_CAR AND B.COD_CTO=CONT.COD_CTO AND GR.COD_EPL=B.COD_EPL AND GR.COD_GRU IN (1,2)  AND B.ESTADO='A';
        END;
    END;        
    /* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ========================================================= BLOQUE 2 ==================================================================                                                                                                                                    
    ...
    BEGIN
        ...
        WHERE B.COD_EPL=CODIGO_EPL AND B.COD_EPL=GR.COD_EPL AND GR.COD_CIU=CI.COD_CIU AND B.COD_DEP=DE.COD_DEP AND B.ESTADO='A';                       
    ...
    BEGIN
        ...
        SELECT CASE WHEN A.COD_BAN=13 THEN SUBSTR(A.COD_SUC,2,3)||A.NUM_CTA ELSE A.NUM_CTA END, C.NOM_BAN, A.TIP_CTA
        INTO CUENTA, NOM_BANCO, TIPO_CUENTA   
        FROM EPL_CONSIGNA A, BANCOS C
        WHERE A.COD_BAN=C.COD_BAN AND A.COD_EPL=CODIGO_EPL AND A.ESTADO='A';           
    ...
    BEGIN
        ...
        BEGIN
            ...
            WHERE COD_EPL=CODIGO_EPL ORDER BY ANO DESC, MES DESC;            
        END;
    END;
    /* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ========================================================= BLOQUE 5 ==================================================================                                                                                                                                    
    ...
    IF (ROL='1') THEN -- EJEMPLO PERFIL GERENTE            
        BLOQUE1 := INITCAP(NOMBRE_EPL||'_*'||CARGO||'_*'||TIP_SALARIO||'_*'||CONTRATO||'_*'||FEC_INGRESO);
        BLOQUE2 := (CORREO||'_*'||TIPO_DOC2||' '||NUM_DOCUMENTO||'_*'||CIUDAD||'_*'||INITCAP(NOM_JEFE||'_*'||DEPARTA));
        BLOQUE3 := (NOM_BANCO||'_*'||CUENTA||'_*'||INITCAP(TIPO_CUENTA2));
        BLOQUE4 := (DEC_RENTA2||'_*'||PROCE_RETEFUEN||'_*'||PORCENRETE||'_*'||CUOTA_MAX);
        BLOQUE5 := NOVEDAD;
        ...
        IF (ROL='2') THEN -- EJEMPLO PERFIL SENA
            BLOQUE1 := INITCAP(NOMBRE_EPL||'_*'||CARGO||'_*'||TIP_SALARIO||'_*'||CONTRATO||'_*'||FEC_INGRESO);
            BLOQUE2 := (CORREO||'_*'||TIPO_DOC2||' '||NUM_DOCUMENTO||'_*'||CIUDAD||'_*'||INITCAP(NOM_JEFE||'_*'||DEPARTA));
            BLOQUE3 := 'INACTIVO';
            BLOQUE4 := 'INACTIVO';
